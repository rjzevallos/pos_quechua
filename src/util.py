...
# Método que entrena una modelo POS Tagging
def train(root_train,root_test,root_output):
    ...
    result, model_outputs, preds_list = model.eval_model(test_data, acc=get_accuracy_and_confusion, output_dir = root_output)
    print(result)
    print("Guardando model POS-Tagging")
    print("Modelo POS-Tagging guardado")
    
    
# Método para usar el modelo NER para un texto    
def use_ner(text):
    model = NERModel("roberta", "Llamacha/pos_quechua")
    prediction, model_output = model.predict([text])
    return prediction
    
    
def use_huggingface(text):
    tokenizer = AutoTokenizer.from_pretrained("Llamacha/pos_quechua")
    model = AutoModelForTokenClassification.from_pretrained("Llamacha/pos_quechua")
